...
def main(args):
    # Normalize mode/if_exists inputs for backwards compatibility
    # If user supplied --mode (allowing Spark-style 'overwrite'), map to pandas/SQLAlchemy style if_exists
    if getattr(args, "mode", None):
        m = args.mode.lower()
        # Accept 'overwrite', 'replace', or 'append'
        if m == "overwrite":
            args.if_exists = "replace"
        elif m in ("replace", "append"):
            args.if_exists = m
        else:
            # unknown mode value — leave if_exists as-is (default will apply)
            pass
...
if __name__ == "__main__":
    ...
    # Accept both --if_exists (pandas-style) AND --mode (spark-style) for flexibility
    parser.add_argument("--if_exists", default="replace", choices=["replace", "append"])
    parser.add_argument("--mode", required=False, choices=["overwrite", "replace", "append"],
                        help="Optional: spark-style mode; 'overwrite' will be mapped to 'replace' for pandas/SQLAlchemy compatibility")
